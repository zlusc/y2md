...
use crate::{AppConfig, OllamaManager, CredentialManager, LlmProviderType};
...
impl Diagnostic {
    pub fn new(name: String, status: DiagnosticStatus, message: String, fix_command: Option<String>) -> Self {
        Self { name, status, message, fix_command }
    }
...
impl DiagnosticReport {
    ...
    pub fn has_errors(&self) -> bool {
        let mut all_diagnostics = self.dependencies.iter()
                        ...
                        if output_dir.exists() {
                            if is_writable(&output_dir) {
                                ...
                                diagnostics.push(Diagnostic::error(
                                    ...
                                    Some(format!("Fix permissions: chmod u+w {}", config.output_dir))
                                ));
                            }
                        } else {
...
After installation: y2md doctor".to_string()
        }
        ("yt-dlp", "macos") => {
...
pub fn print_diagnostic_report(report: &DiagnosticReport) {
    ...
    let status_text = if report.has_errors() {
        ...
        style("Overall Status: ⚠ Ready with warnings").yellow().bold()
    } else {
...
fn print_section(title: &str, diagnostics: &[Diagnostic], term: &console::Term) {
    ...
    for diagnostic in diagnostics {
        let (symbol, color_fn): (String, fn(console::StyledObject<String>) -> console::StyledObject<String>) = match diagnostic.status {
...
fn print_suggestions(report: &DiagnosticReport, term: &console::Term) {
    let all_diagnostics = report.dependencies.iter()
