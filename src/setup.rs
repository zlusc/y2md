...
use console::style;
use y2md::{AppConfig, LlmSettings, LlmProviderType, LocalLlmConfig, OpenAiConfig, 
           AnthropicConfig, DeepSeekConfig, CustomLlmConfig, OllamaManager, CredentialManager};
...
impl SetupWizard {
    pub async fn run() -> Result<AppConfig> {
        ...
        println!("Configuration saved to: {}", AppConfig::config_path()?.display());
        println!("\n{}", style("Next steps:").bold());
        println!("  1. Test your setup: {}", style("y2md doctor").cyan());
        println!("  2. Transcribe a video: {}", style("y2md <YOUTUBE_URL>").cyan());
    ...
    fn prompt_output_directory() -> Result<String> {
        ...
        if !std::path::Path::new(&expanded).exists() {
            if Confirm::new()
                .with_prompt(format!("Directory '{}' doesn't exist. Create it?", expanded))
                .default(true)
                .interact()?
            {
    ...
    async fn prompt_llm_setup() -> Result<LlmSettings> {
        ...
        match selection {
            ...
            5 => {
                ...
                println!("  You can enable it later with: {}\n", style("y2md setup-llm").cyan());
                Ok(LlmSettings {
                    enabled: false,
                    ..Default::default()
                })
    ...
    async fn setup_ollama() -> Result<LlmSettings> {
        ...
        if !ollama.is_available().await {
            println!("  {} Ollama is not running or not installed.", style("⚠").yellow());
            println!();
            ...
            match choice {
                0 => {
                    ...
                    println!("\n  After installing Ollama, run: {}", style("y2md init").cyan());
                ...
                1 => {
                    ...
                    if !ollama.is_available().await {
                        ...
                        println!("  Start Ollama manually, then run: {}", style("y2md init").cyan());
        ...
        if models.is_empty() {
            ...
            println!("  • {} - Fast, small download (2GB)", style("llama3.2:3b").cyan());
            println!("  • {} - High quality (7GB)", style("mistral-nemo:12b").cyan());
            println!();
            ...
            if let Err(e) = ollama.download_model(model_name).await {
                ...
                println!("  Try downloading manually: {}", style(format!("ollama pull {}", model_name)).cyan());
                return Ok(LlmSettings {
                    enabled: false,
                    ..Default::default()
                });
            ...
            println!("  {} Using model: {}", style("✓").green(), style(&models[model_idx]).cyan());
            println!();
    ...
    async fn setup_openai() -> Result<LlmSettings> {
        ...
        match response {
            ...
            Ok(resp) => {
                println!("  {} Invalid API key or API error: {}", style("✗").red(), resp.status());
        ...
        println!("\n  {} OpenAI configured with {}", style("✓").green(), style(model_name).cyan());
        println!();
    ...
    async fn setup_anthropic() -> Result<LlmSettings> {
        ...
        match response {
            ...
            Err(e) => {
                println!("  {} Could not connect to Anthropic: {}", style("✗").red(), e);
                return Err(anyhow::anyhow!("Connection error"));
            }
        }
        ...
        println!("\n  {} Anthropic configured with {}", style("✓").green(), style(model_name).cyan());
        println!();
    ...
    async fn setup_custom() -> Result<LlmSettings> {
        println!("{}", style("Setting up Custom OpenAI-compatible API").bold());
        println!();
